...
#define __builtin_clzl(t) msbindex32(t)
  ...
  struct bitvecbase {
    ...
    bitvecbase(const bitvecbase<N>& vec) { foreach (i, (int)N) w[i] = vec.w[i]; }
    ...
    bool equalop(const bitvecbase<N>& x) const {
      T t = 0;
      foreach (i, (int)N) { t |= (w[i] ^ x.w[i]); }
      return (t == 0);
    }

    bool nonzeroop() const {
      T t = 0;
      foreach (i, (int)N) { t |= w[i]; }
      return (t != 0);
    }
